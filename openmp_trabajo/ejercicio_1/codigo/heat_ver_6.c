...
#pragma omp parallel private(k,i)
{
  ...
  {
    it++;
    continuar_bucle=(it<=itmax);
    if(it>2 && dphimax<eps)
      continuar_bucle=0;
    dphimax=0.;
  }

  if(continuar_bucle!=0)
  {
    #pragma omp for schedule(static)
    for (i=1;i<imax;i++)
      for (k=1;k<kmax;k++)
      {
        phin[i][k]=((phi[i+1][k]+phi[i-1][k]-2.*phi[i][k])*dy2i
            +(phi[i][k+1]+phi[i][k-1]-2.*phi[i][k])*dx2i)*dt;
      }

    /* save values */
    #pragma omp for schedule(static) nowait reduction(max:dphimax)
    for (i=1;i<imax;i++)
      for (k=1;k<kmax;k++){
        phi[i][k]+=phin[i][k];
        dphimax=max(dphimax,phin[i][k]);
      }
